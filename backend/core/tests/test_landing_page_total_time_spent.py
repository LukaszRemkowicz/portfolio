...
class TestLandingPageTotalTimeSpentTask:
    def test_calculate_task_updates_image_and_total(self, mocker) -> None:
        LandingPageSettingsFactory()
        mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
    ...
    def test_calculate_task_creates_settings_when_missing(self, mocker) -> None:
        mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        ...
        assert result["total_time_spent"] == 1.5

    def test_recalculate_task_sums_image_hours(self, mocker) -> None:
        mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        mock_invalidate_cache = mocker.patch(
            "core.tasks.CacheService.invalidate_landing_page_cache"
        )
        mock_invalidate_ssr = mocker.patch("core.tasks.invalidate_frontend_ssr_cache_task.delay")
        LandingPageSettingsFactory()
        AstroImageFactory(calculated_exposure_hours=1.5)
        AstroImageFactory(calculated_exposure_hours=2.25)
        mock_invalidate_cache.reset_mock()
        mock_invalidate_ssr.reset_mock()

        result = recalculate_landing_page_total_time_spent_task()

        assert result["total_time_spent"] == 3.75
...
class TestLandingPageTotalTimeSpentSignals:
    def test_translation_save_queues_task_with_exposure_details(self, mocker) -> None:
    ...
    def test_translation_save_skips_empty_exposure_details(self, mocker) -> None:
        mock_delay = mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
    ...
    def test_translation_save_does_not_queue_when_exposure_details_is_unchanged(
        self, mocker
    ) -> None:
        mock_delay = mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
    ...
    def test_translation_save_does_not_queue_when_only_other_field_changes(self, mocker) -> None:
        mock_delay = mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        image = AstroImageFactory(exposure_details="2h total")
    ...
    def test_translation_save_queues_when_exposure_details_changes(self, mocker) -> None:
        mock_delay = mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        image = AstroImageFactory(exposure_details="2h total")
    ...
    def test_translation_save_for_non_default_language_does_not_queue(self, mocker) -> None:
        mock_delay = mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        image = AstroImageFactory(exposure_details="2h total")
    ...
    def test_astroimage_delete_requeues_total_recalculation(self, mocker) -> None:
        mocker.patch(
            "astrophotography.signals.calculate_astroimage_exposure_hours_task.delay_on_commit"
        )
        mock_delay = mocker.patch(
            "astrophotography.signals."
            "recalculate_landing_page_total_time_spent_task.delay_on_commit"
        )
        ...
        mock_delay.assert_called_with(image_id)
...
class TestLandingPageTotalTimeSpentCommand:
    def test_command_calculates_only_images_without_hours_by_default(self, mocker) -> None:
        ...
        AstroImageFactory(exposure_details="second", calculated_exposure_hours=7.0)

        def fake_task(astro_image_id: str):
            mapping = {
                str(first.pk): 1.5,
            }
            type(first).objects.filter(pk=astro_image_id).update(
                calculated_exposure_hours=mapping[astro_image_id]
            )
            return {
                "status": "success",
                "astro_image_id": astro_image_id,
                "parsed_hours": mapping[astro_image_id],
                "total_time_spent": mapping[astro_image_id],
            }
        ...
        mock_recalculate_total = mocker.patch(
            "core.management.commands.recalculate_landing_page_total_time_spent."
            "recalculate_landing_page_total_time_spent_task",
            side_effect=recalculate_landing_page_total_time_spent_task,
        )
        output = StringIO()

        call_command("recalculate_landing_page_total_time_spent", stdout=output)

        assert mock_calculate_task.call_count == 1
        mock_calculate_task.assert_called_once_with(str(first.pk))
        assert mock_recalculate_total.called

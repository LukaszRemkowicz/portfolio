...
class TestConvertImagesToWebpCommand:
    def test_command_converts_images(self, mocker: MockerFixture) -> None:
        ...
        assert astro_img.original_image is not None
    ...
    def test_command_dry_run_no_changes(self, mocker: MockerFixture) -> None:
        ...
        assert not astro_img.original_image
    ...
    def test_command_force_reconversion(self, mocker: MockerFixture) -> None:
        ...
        astro_img.refresh_from_db()
        assert astro_img.path.name.endswith(".webp")
        assert astro_img.original_image == "test.jpg"
        ...
        # 3. Third run with force - should reconvert from original_image
        call_command("convert_images_to_webp", force=True)
        # Verify it was called with original_image as source
        assert mock_convert_to_webp.called
        # The source should be the original_image
    ...
    def test_command_object_id_limits_conversion_to_single_object(
        ...
        assert target.path.name.endswith(".webp")
        assert target.original_image is not None
        assert other.path.name == other_path_before
        assert other.original_image in (None, "")
        assert target.path.name != target_path_before
        mock_call_command.assert_any_call("clear_cache")
    ...
    def test_command_object_ids_limits_conversion_to_requested_objects(
        ...
        assert first.path.name.endswith(".webp")
        assert first.original_image is not None
        assert second.path.name.endswith(".webp")
        assert second.original_image is not None
        assert third.path.name == third_path_before
        assert third.original_image in (None, "")
        mock_call_command.assert_any_call("clear_cache")

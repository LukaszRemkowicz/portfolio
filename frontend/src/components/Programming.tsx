// frontend/src/components/Programming.tsx
import { type FC, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { useProjects } from '../hooks/useProjects';
import styles from '../styles/components/Programming.module.css';
import { Github, ExternalLink, Code2 } from 'lucide-react';
import ProjectSkeleton from './skeletons/ProjectSkeleton';
import ImageWithFallback from './common/ImageWithFallback';
import { Project, ProjectImage } from '../types';
import StarBackground from './StarBackground';

const ProjectCard: FC<{ project: Project }> = ({ project }) => {
  const { t } = useTranslation();
  const [hasError, setHasError] = useState(false);
  const coverImage =
    project.images.find((img: ProjectImage) => img.is_cover) ||
    project.images[0];

  return (
    <article className={styles.projectCard}>
      <div className={styles.imageWrapper}>
        {coverImage ? (
          <ImageWithFallback
            src={coverImage.url}
            alt={project.name}
            className={styles.cardImage}
            onError={() => setHasError(true)}
          />
        ) : (
          <div className={styles.imagePlaceholder}>
            <Code2 size={48} className={styles.placeholderIcon} />
          </div>
        )}
        {hasError && (
          <div className={styles.placeholderOverlay}>
            <span className={styles.placeholderText}>[Placeholder]</span>
          </div>
        )}
      </div>

      <div className={styles.cardContent}>
        <h3 className={styles.cardTitle}>{project.name}</h3>
        <p className={styles.cardDescription}>{project.description}</p>

        <div className={styles.techStack}>
          {project.technologies_list.map((tech: string, index: number) => (
            <span key={index} className={styles.techBadge}>
              {tech}
            </span>
          ))}
        </div>

        <div className={styles.cardActions}>
          {project.github_url && (
            <a
              href={project.github_url}
              target='_blank'
              rel='noopener noreferrer'
              className={styles.actionLink}
            >
              <Github className={styles.icon} /> {t('programming.source')}
            </a>
          )}
          {project.live_url && (
            <a
              href={project.live_url}
              target='_blank'
              rel='noopener noreferrer'
              className={styles.actionLink}
            >
              <ExternalLink className={styles.icon} />{' '}
              {t('programming.liveDemo')}
            </a>
          )}
        </div>
      </div>
    </article>
  );
};

const Programming: FC = () => {
  const { t } = useTranslation();
  const {
    data: projects = [],
    isLoading: loading,
    error: queryError,
  } = useProjects();
  const error = queryError ? 'Failed to compile projects.' : null;

  if (error) {
    return (
      <div className={styles.error}>
        <p>{error}</p>
      </div>
    );
  }

  return (
    <>
      <StarBackground />
      <section className={styles.section}>
        <header className={styles.header}>
          <h1 className={styles.title}>{t('programming.title')}</h1>
          <p className={styles.subtitle}>{t('programming.subtitle')}</p>
        </header>

        <div className={styles.grid}>
          {loading ? (
            <ProjectSkeleton count={3} />
          ) : projects.length > 0 ? (
            projects.map(project => (
              <ProjectCard key={project.pk} project={project} />
            ))
          ) : (
            <div className={styles.noResults}>
              <p>{t('programming.empty')}</p>
            </div>
          )}
        </div>
      </section>
    </>
  );
};

export default Programming;
